...
def check_asteroid_ephemeris(
    ...
    if not bsp_filepath.exists():
        raise FileNotFoundError(
            f"Asteroid ephemeris file '{bsp_filepath.name}' not found in '{bsp_filepath.parent}'."
        )
    ...
    if not (bsp_start < required_start and bsp_end > required_end):
        raise ValueError(
...
def check_uncertainty_file(uncertainty_filepath: pathlib.Path) -> bool:
    ...
    if not uncertainty_filepath.exists():
        raise FileNotFoundError(
            f"Uncertainty file '{uncertainty_filepath.name}' not found in '{uncertainty_filepath.parent}'."
        )
