	  ...
	  PARAMETER (DPI=3.141592653589793238462643D0,DIFMAX=6D0)
...
C	  WRITE(STDOUT,5000)
          M = IARGC()
          IF (M.NE.1) PAUSE 'ONLY ONE ARGUMENT REQUIRED'
...
2	  CLOSE(1)
	  ...
	  DO J=1,I
           IF (INDX(J).EQ.1) THEN
            ...
            WRITE(STDOUT,'(F13.8,1X,F13.8)')
     +      RA(J)*180D0/DPI,DEC(J)*180D0/DPI

c      	    WRITE(STDOUT,'(I2.2,1X,I2.2,1X,I2.2,A1,I4.4,2X,
c     +                     A1,I2.2,1X,I2.2,1X,I2.2,A1,I3.3)')
c     +      IHMSF(1),IHMSF(2),IHMSF(3),'.',IHMSF(4),
c     +      SIGN,IDMSF(1),IDMSF(2),IDMSF(3),'.',IDMSF(4)
	   ENDIF
	  END DO
