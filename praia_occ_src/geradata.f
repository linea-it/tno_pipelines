      ...
      DOUBLE PRECISION D2S,T0,T1
      PARAMETER (D2S=86400.0D0) ! numero de segundos em 24hs
      PARAMETER (T0=2451545.0D0) ! data juliana TDB da epoca J2000.0
      PARAMETER (T1=2400000.5D0) ! MJD=JD-T1

      CHARACTER*30 CALENDI,CALENDF,CALEND
      CHARACTER*128 KERNEL
      DOUBLE PRECISION ETOBS,ETOBSF,PASSO

      DOUBLE PRECISION UTA,UTB,JDUTC,JDTT
      DOUBLE PRECISION DATE1,DATE2,TDB,UT1,
     +                 ELONG,U,V
      DOUBLE PRECISION iau_DTDB,sla_DTT,DTB,DTT,MJD
...
c      WRITE(STDOUT,1100)
      READ(STDIN,'(A30)')CALENDF
c      WRITE(STDOUT,1200)
      READ(STDIN,*)PASSO


      CALL STR2ET (CALENDI,ETOBS)
      CALL STR2ET (CALENDF,ETOBSF)

1     CONTINUE

       CALL ET2UTC (ETOBS,'C',3,CALEND)

       IF (ETOBS-ETOBSF.GT.1E-5) GOTO 2
       WRITE(STDOUT,'(A4,A1,A3,A1,A2,A1,A12)')
...
c       write(6,'(f20.10)')ETOBS-ETOBSF
       ...
       UT1=MOD(JDUTC,1D0) ! IGNORADA A DIFERENCA UTC-UT1
       ELONG=0D0
       U=0D0
       V=0D0

       DTB=iau_DTDB(DATE1,DATE2,UT1,ELONG,U,V)
...
C TRANSFORMA "ET" - ESTILO SPICE - PARA DATA
C JULIANA, COM AUXILIO DA ROTINA ET2UTC E ROTINAS
C SLALIB. ISTO E' FEITO PORQUE NAO CONSIGO EXTRAIR
C DATA JULIANA COM 9 DIGITOS APOS A VIRGULA DA
C ROTINA ET2UTC DIRETAMENTE

      IMPLICIT NONE

      DOUBLE PRECISION T1
      PARAMETER (T1=2400000.5D0) ! offset para mjd (jd=mjd+T1)

      INTEGER NSTRT,JFLAG
      INTEGER IY,IM,ID
      DOUBLE PRECISION HORA,MINU,SEGU,X
      DOUBLE PRECISION ET,MJD,JD
      CHARACTER*30 CALEND

      CALL ET2UTC(ET,'ISOC',3,CALEND)
      CALEND(5:5)=' '
      CALEND(8:8)=' '
      CALEND(11:11)=' '
      CALEND(14:14)=' '
      CALEND(17:17)=' '
      NSTRT=1
      CALL sla_DFLTIN (CALEND,NSTRT,X,JFLAG)
      IY=DINT(X)
      CALL sla_DFLTIN (CALEND,NSTRT,X,JFLAG)
      IM=DINT(X)
      CALL sla_DFLTIN (CALEND,NSTRT,X,JFLAG)
      ID=DINT(X)
      CALL sla_CALDJ(IY,IM,ID,MJD,JFLAG)
      JD=MJD+T1
      CALL sla_DFLTIN (CALEND,NSTRT,HORA,JFLAG)
      CALL sla_DFLTIN (CALEND,NSTRT,MINU,JFLAG)
      CALL sla_DFLTIN (CALEND,NSTRT,SEGU,JFLAG)
      JD=JD+HORA/24D0+MINU/1440D0+SEGU/86400D0
...
*     :                                         :
*     :  Julio Camargo - ON/MCT - 2009/DEC/22   :
*     :-----------------------------------------:
*     :  User note:                             :
*     :  Latest leap second: 2012 July 1        :
*     :                                         :
*     :  Julio Camargo - ON/MCT - 2012/JAN/18   :
*     :-----------------------------------------:
*     :  User note:                             :
*     :  Latest leap second: 2015 July 1        :
*     :                                         :
*     :  Julio Camargo - ON/MCT - 2015/JAN/12   :
*     :-----------------------------------------:
*     :-----------------------------------------:
*     :  User note:                             :
*     :  Latest leap second: 2017 January 1     :
*     :                                         :
*     :  Julio Camargo - ON/MCTI- 2016/AUG/02   :
*     :-----------------------------------------:
*
...
*     2017  January 1 (source:http://maia.usno.navy.mil/ser7/tai-utc.dat)
      ELSE IF (UTC.GE.57754D0) THEN
         DT=37D0

*     2015  July 1 (source:http://maia.usno.navy.mil/ser7/tai-utc.dat)
      ELSE IF (UTC.GE.57204D0) THEN
         DT=36D0

*     2012  July 1 (source:http://maia.usno.navy.mil/ser7/tai-utc.dat)
      ELSE IF (UTC.GE.56109D0) THEN
         DT=35D0

*     2009 January 1 (source:http://maia.usno.navy.mil/ser7/tai-utc.dat)
      ELSE IF (UTC.GE.54832D0) THEN
         DT=34D0

*     2006 January 1 (source:http://maia.usno.navy.mil/ser7/tai-utc.dat)
